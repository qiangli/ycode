---
name: analyze
description: Autonomously analyze an agentic tool, identify gaps in three focused domains, implement, test, and commit — fully unattended
user_invocable: true
---

# /analyze — Autonomous Agentic Tool Analysis
...
> Use `/learn` if you want research only (gap analysis + plan + TODO, no code changes).
> `/analyze` runs the full implement-and-commit cycle. For arbitrary
> development tasks unrelated to comparing against another tool, use
> `/autopilot fix|improve|task <goal>`.
