...
- **Podman engine relocation (2026-06-27, AgentOS Phase 4):** ycode's in-process podman engine (`internal/container` + `pkg/oci`) was **copied into `../coreutils/external/podman/engine` + `../coreutils/pkg/oci`** (consuming the same `qiangli/podman` fork) so `bashy podman` / outpost embed an isolated podman without ycode. coreutils is now the **canonical home**. ycode **still carries its own copy** — `internal/container` has 17 in-ycode consumers (sandbox, toolexec, serve, mcp, …), so it's the container engine, not a mere duplicate. The planned dedup is to turn `internal/container` into thin **alias shims** re-exporting `coreutils/external/podman/engine` (the same pattern as `internal/runtime/{treesitter,repomap,codegraph}`) — a focused, fully-tested refactor, deliberately deferred. Until then both copies coexist; keep podman behavior changes in sync or land them in coreutils and re-shim. See dhnt/docs/agentos-substrate-extraction-plan.md + local-p2p-cicd.md.
