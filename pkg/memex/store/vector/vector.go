...
// Store implements store.VectorStore backed by chromem-go.
type Store struct {
	mu            sync.RWMutex
	db            *chromem.DB
	dir           string
...
func WithEmbeddingFunc(fn store.EmbeddingFunc) Option {
	return func(s *Store) {
...
func (s *Store) AddDocuments(ctx context.Context, collection string, docs []store.VectorDocument) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) Query(ctx context.Context, collection string, queryEmbedding []float32, maxResults int) ([]store.SearchResult, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) QueryByText(ctx context.Context, collection string, query string, maxResults int) ([]store.SearchResult, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func toSearchResults(results []chromem.Result) []store.SearchResult {
