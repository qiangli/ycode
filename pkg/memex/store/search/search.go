...
// Store implements store.SearchIndex backed by Bleve.
type Store struct {
	mu      sync.RWMutex
	dir     string
...
func (s *Store) Index(_ context.Context, indexName string, doc store.Document) error {
...
func (s *Store) BatchIndex(_ context.Context, indexName string, docs []store.Document) error {
	idx, err := s.getOrCreateIndex(indexName)
	if err != nil {
		return err
	}
...
func (s *Store) Search(_ context.Context, indexName string, query string, maxResults int) ([]store.SearchResult, error) {
	idx, err := s.getOrCreateIndex(indexName)
	if err != nil {
		return nil, err
	}
...
func (s *Store) SearchWithFilter(_ context.Context, indexName string, query string, filters map[string]string, maxResults int) ([]store.SearchResult, error) {
	idx, err := s.getOrCreateIndex(indexName)
	if err != nil {
		return nil, err
	}
...
var _ store.SearchIndex = (*Store)(nil)
