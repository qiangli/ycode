...
go build -trimpath -tags "sqlite,sqlite_unlock_notify" -ldflags "-s -w -X main.version=${VERSION} -X main.commit=${COMMIT}" -o bin/ycode ./cmd/ycode/
if [ "$(uname)" = "Darwin" ]; then codesign -f -s - bin/ycode 2>/dev/null || true; fi

echo "=== Step 5: Unit tests ==="
go test -short -race -tags "sqlite,sqlite_unlock_notify" ${PACKAGES}
