...
func parseSwitchArgs(args string) (selector string, fresh bool, err error) {
	for _, f := range strings.Fields(args) {
		switch {
		case f == "--fresh" || f == "--no-context":
			fresh = true
		case strings.HasPrefix(f, "-"):
			return "", false, fmt.Errorf("unknown flag %q (only --fresh is supported)", f)
		case selector == "":
			selector = f
		default:
			return "", false, fmt.Errorf("unexpected argument %q", f)
		}
	}
...
func registerSwitchCommands(r *Registry, deps *RuntimeDeps) {
	r.Register(&Spec{
		...
		Usage:       "/agent [name|nick|L3] [--fresh]",
		Category:    "session",
		Examples: []string{
			"switch to codex",
			"hand this to claude",
...
func switchHandler(deps *RuntimeDeps, verb string) HandlerFunc {
	return func(ctx context.Context, args string) (string, error) {
		selector, fresh, err := parseSwitchArgs(args)
		if err != nil {
			return "", err
		}
		...
		req := SwitchRequest{Verb: verb, Fresh: fresh}
		if verb == "tool" {
			req.Tool = selector
		} else {
			req.Agent = selector
...
func listSwitchTargets(verb string) string {
	...
	b.WriteString("\nUsage: /agent <name|nick|L3> [--fresh]\n" +
		"The current conversation is carried across unless --fresh is given.\n")
	return b.String()
}
