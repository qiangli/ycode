...
func buildLongFormDoc() string {
	...
	b.WriteString("This file is auto-generated by `ycode init`. ycode is a local, YAML-native agent harness driven entirely by `agent.yaml` (see the block below). It does not run an MCP server and installs no shell built-ins; there is nothing to add to your tool's `mcpServers` config.\n\n")
	...
	b.WriteString("Network frontends exist only when `agent.yaml` declares them; start them with `ycode serve --file agent.yaml`. Live endpoints are advertised in `~/.agents/ycode/manifest.json`.\n\n")
	b.WriteString("To refresh ycode's breadcrumbs in this repo:\n\n")
	b.WriteString("```\nycode init --refresh\n```\n\n")
	b.WriteString("That regenerates this file (and, with the foreign-tool opt-in, the delimited block in your tool's memory file).\n")
	return b.String()
}
