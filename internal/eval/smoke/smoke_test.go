...
func TestSmoke(t *testing.T) {
	...
	if err != nil {
		t.Skipf("skipping smoke evals: %v", err)
	}
	...
	for _, scenario := range Scenarios() {
		...
		t.Run(scenario.Name, func(t *testing.T) {
			if scenario.Policy == eval.UsuallyPasses && os.Getenv("RUN_USUALLY_PASSES") == "" {
				t.Skip("skipping UsuallyPasses scenario (set RUN_USUALLY_PASSES=1)")
			}
			...
			if passed < threshold {
				...
				t.Logf("%s: %d/%d trials passed, pass@k=%.2f, pass^k=%.2f, flakiness=%.2f",
					scenario.Name, passed, len(result.Trials),
					result.Metrics.PassAtK, result.Metrics.PassPowK, result.Metrics.Flakiness)
			}
		})
	}
}
