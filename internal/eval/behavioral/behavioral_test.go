...
func TestBehavioral(t *testing.T) {
	...
	if err != nil {
		t.Skipf("skipping behavioral evals: %v", err)
	}
	...
	for _, scenario := range Scenarios() {
		...
		t.Run(scenario.Name, func(t *testing.T) {
			...
			if passed < threshold {
				t.Errorf("%s: %d/%d trials passed (need %d), pass@k=%.2f, trajectory=%.2f",
					scenario.Name, passed, len(result.Trials), threshold,
					result.Metrics.PassAtK, result.Metrics.TrajectoryScore)
			} else {
				t.Logf("%s: %d/%d passed, pass@k=%.2f, pass^k=%.2f, flakiness=%.2f, trajectory=%.2f",
					scenario.Name, passed, len(result.Trials),
					result.Metrics.PassAtK, result.Metrics.PassPowK,
					result.Metrics.Flakiness, result.Metrics.TrajectoryScore)
			}
		})
	}
}
