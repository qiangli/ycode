...
- **Memory interop:** Expose ycode's memory layers to the broader agent ecosystem. Today the surface is `yc remember` / `yc recall` plus the on-disk `MEMORY.md` files; MCP is not an option (removed — see [plan-remove-mcp.md](./plan-remove-mcp.md)).
