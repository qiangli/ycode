...
// `ycode docs` (curated capability prompts).
//
// Two surfaces are enumerated:
//
...
func newToolsCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "tools",
		Short: "List the tool surfaces this binary exposes (internal, CLI)",
		Long: "Operator-side inventory of ycode's tool surfaces. Use --internal / --cli " +
			"to scope, --json for machine-readable, --filter to substring-match names. " +
			"For the agent-facing equivalents see `ycode docs`.",
	}
	cmd.AddCommand(&cobra.Command{
		Use:          "list",
		Short:        "List tools across internal / CLI surfaces",
		SilenceUsage: true,
		RunE: func(c *cobra.Command, _ []string) error {
			// Default to all surfaces when no scope flag set; --all is
			// retained as an explicit synonym for scripts that want to
			// document intent.
			if !showInternal && !showCLI {
				showAll = true
			}
			scopes := scopeSet{internal: showInternal || showAll, cli: showCLI || showAll}
	...
	cmd.PersistentFlags().BoolVar(&showAll, "all", false, "Equivalent to --internal --cli (default when no scope flag)")
...
type toolRow struct {
	Surface     string `json:"surface"`     // "internal" | "cli"
	Name        string `json:"name"`        // tool name (internal) or verb (cli)
	Permission  string `json:"permission"`  // ReadOnly / WorkspaceWrite / DangerFullAccess / "" for cli
	Source      string `json:"source"`      // spec source (internal), or "" (cli)
