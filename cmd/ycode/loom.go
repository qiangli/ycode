...
func buildLoomComponent(_ context.Context, client *gitserver.Client, token, giteaDataDir string) (*loomComponent, *loompkg.Service, error) {
	...
	svc, err := loompkg.NewService(loompkg.Options{
		...
		Logger:      slog.Default(),
		// OnLeaseCwd is intentionally nil: ycode does not modify a
		// foreign repo just because someone leased a workspace inside
		// it. To establish ycode in a repo, the user runs `ycode init`
		// explicitly.
	})
	if err != nil {
		mergerCancel()
