...
hints to stderr, YCODE_SHELL_AGENT=1 env var). See docs/shell-agent.md.`,
	...
	cmd.Flags().BoolVar(&f.sandbox, "sandbox", false, "Advisory only — sandbox routing is not yet wired; use 'yc sandbox -- <cmd>' for per-call podman isolation")
	cmd.Flags().StringSliceVar(&f.allowedDirs, "allowed-dirs", nil, "Advisory only — full VFS enforcement is not yet wired in shell mode")
