...
- Git: `internal/runtime/toolexec/` — 31 native go-git NativeFuncs (3-tier: native → host exec → container). Shell: `internal/runtime/bash/` — in-process mvdan/sh interpreter with security ExecHandler middleware (Setpgid, pre-exec validation).
...
When the user's message starts with `/<name>`, read `skills/<name>/skill.md` and follow it. Everything after `/<name> ` is `ARGS`. Project skills: `/analyze`, `/audit`, `/autopilot`, `/build`, `/claude`, `/deploy`, `/eval`, `/learn`, `/oci`, `/setup`, `/validate`, `/bench-instructions`. Some skills (`/init`, `/commit`) are embedded in the binary.
